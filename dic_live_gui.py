...
class DICLive(QtWidgets.QMainWindow):
    ...
    def measure_L0(self):
        ...
        L0_mm = pd * self.scale
        self.L0_edit.setText(f"{L0_mm:.3f}")
        self.info_label.setText(f"L₀ set: {L0_mm:.3f} mm")
    ...
    def measure_W0(self):
        ...
        pd = np.hypot(pts[1][0]-pts[0][0], pts[1][1]-pts[0][1])
        W0_mm = pd * self.scale
        self.W0_edit.setText(f"{W0_mm:.3f}")
        self.info_label.setText(f"W₀ set: {W0_mm:.3f} mm")
    ...
    def set_reference(self):
        try:
            ...
            if self.ref_pts is None:
                self.auto_seeds()
            self.frame_count=0; self.cum_dx_px=None; self.cum_dy_px=None
            self.info_label.setText(f"Reference set ({len(self.ref_pts)} pts)")
        except Exception:
    ...
    def update_frame(self):
        try:
            ...
            if self.diff_chk.isChecked() and self.frame_count>=self.diff_interval.value():
                self.cum_dx_px+=dx_px; self.cum_dy_px+=dy_px
                self.ref_gray=cur.copy()
                self.ref_pts[:,0,0]+=raw_dx; self.ref_pts[:,0,1]+=raw_dy
                self.frame_count=0
            ...
            if self.diff_chk.isChecked():
                ...
                disp_dx_px=dx_px; disp_dy_px=dy_px
            ...
            if met=="Disp X (mm)":
                ...
                if met=="Axial Strain":
                    ...
                    with np.errstate(divide='ignore',invalid='ignore'):
                        p=-et/ea
                    field=np.nan_to_num(p,0.0,0.0,0.0)

            # structured-grid or sparse fallback
            ...
            # draw L0 and W0 lines if set
            def draw_line(pts,color):
                (x1,y1),(x2,y2)=pts
                cv2.line(disp,(x1,y1),(x2,y2),color,2)
            # annotate with current avg strains
            if self.L0_pts:
                avg_ax = float(np.nanmean(dy_mm)) / float(self.L0_edit.text())
                draw_line(self.L0_pts,(0,255,255))
                mid = ((self.L0_pts[0][0]+self.L0_pts[1][0])//2,
                       (self.L0_pts[0][1]+self.L0_pts[1][1])//2)
                cv2.putText(disp,f"ε_ax={avg_ax:.3e}",mid,cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,255),1)
            if self.W0_pts:
                avg_tr = float(np.nanmean(dx_mm)) / float(self.W0_edit.text())
                draw_line(self.W0_pts,(0,255,0))
                mid = ((self.W0_pts[0][0]+self.W0_pts[1][0])//2,
                       (self.W0_pts[0][1]+self.W0_pts[1][1])//2)
                cv2.putText(disp,f"ε_tr={avg_tr:.3e}",mid,cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,0),1)
