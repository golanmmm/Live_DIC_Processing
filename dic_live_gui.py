#!/usr/bin/env python3
# ------------------------------------------------------------------
# Live-DIC GUI  –  Robust full-file implementation
# ------------------------------------------------------------------
...
class DICLive(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # gauge
        self.gauge_pts = None       # [(x1,y1),(x2,y2)] in full-image coords
        ...
        self.spin_ks    = QtWidgets.QSpinBox(); self.spin_ks.setRange(1,51); self.spin_ks.setSingleStep(2); self.spin_ks.setValue(11)
        ...
        for row in [
            ...
            if len(row)==2: form.addRow(row[0], row[1])
            else:           form.addRow(row[0])
    ...
    def select_roi(self):
        ...
        if w>0 and h>0:
            ...
            self.mask_roi = (self.mask_full[y:y+h, x:x+w]
                             if self.mask_full is not None else None)
            self.ref_gray = None
        ...
        if shape=="rect":
            ...
            def cb(evt, xx, yy, flags, prm):
                if evt==cv2.EVENT_LBUTTONDOWN and len(pts)<2:
                    pts.append((xx,yy))
                    cv2.circle(img,(xx,yy),4,(0,0,255),-1)
    ...
    def auto_seed(self):
        ...
        mask = (self.mask_full[y:y+h, x:x+w]
                if self.mask_full is not None else None)
    ...
    def select_gauge(self):
        ...
        def cb(evt, xx, yy, flags, prm):
            if evt==cv2.EVENT_LBUTTONDOWN and len(pts)<2:
                pts.append((xx,yy))
                cv2.circle(img,(xx,yy),5,(255,255,255),-1)
        ...
        if len(pts)==2:
            self.gauge_pts = pts
            dx_pix = pts[1][0]-pts[0][0]
            dy_pix = pts[1][1]-pts[0][1]
            L0_pix = np.hypot(dx_pix, dy_pix)
            self.gauge_L0_mm = L0_pix * self.scale_mm
            self.gauge_lbl.setText(f"Gauge L0 = {self.gauge_L0_mm:.3f} mm")
    ...
    def calibrate(self):
        ...
        if len(pts)==2:
            ...
            if ok and pix>0:
                ...
                self.info.setText(f"Calibrated: {self.scale_mm:.6f} mm/px")
    ...
    def process(self):
        ...
        try:
            ...
            # compute gauge strain if set
            gauge_text = ""
            if self.gauge_pts is not None and self.gauge_L0_mm is not None:
                # convert to ROI coords
                (gx1,gy1),(gx2,gy2) = self.gauge_pts
                rx,ry,_w,_h = self.roi
                r1 = (gy1-ry, gx1-rx)
                r2 = (gy2-ry, gx2-rx)
                # clamp
                for (ry_,rx_) in (r1,r2):
                    if not (0<=ry_<h and 0<=rx_<w):
                        gauge_text="Gauge OOB"; break
                else:
                    d0 = np.array([ux[r1], uy[r1]])
                    d1 = np.array([ux[r2], uy[r2]])
                    dx_mm = ( (gx2-gx1)*self.scale_mm + (d1[0]-d0[0]) )
                    dy_mm = ( (gy2-gy1)*self.scale_mm + (d1[1]-d0[1]) )
                    L1 = np.hypot(dx_mm, dy_mm)
                    if self.gauge_L0_mm>0:
                        eps_g = (L1 - self.gauge_L0_mm)/self.gauge_L0_mm
                        gauge_text = f"Gauge ε = {eps_g*100:.3f} %"
                    # draw gauge line
                    cv2.line(vis, (gx1,gy1), (gx2,gy2), (255,255,255), 2)
                    mid = ((gx1+gx2)//2, (gy1+gy2)//2)
                    cv2.putText(vis, f"{eps_g*100:.2f}%", mid,
                                cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),1)
            self.gauge_lbl.setText(gauge_text)
            ...
            cv2.putText(leg, metric, (5,30),
                        cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,255,255),1)
            cv2.putText(leg, f"{vmax:.3g} {unit}", (5,55),
                        cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),1)
            cv2.putText(leg, f"{vmin:.3g} {unit}", (5,H-10),
                        cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),1)
