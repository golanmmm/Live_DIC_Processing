...
class DICLive(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Live DIC GUI By Golan Michaeli")
    ...
    def toggle_record(self):
        if not self.recording:
            ...
            if not fname:
                ...
                ext = ".avi" if fmt == "Uncompressed AVI" else ".mp4"
                fname = f"LiveDIC_{dt}{ext}"
        ...
        try:
            ...
            if self.gauge_pts and self.gauge_L0_mm:
                ...
                if 0<=ry1<h and 0<=rx1<w and 0<=ry2<h and 0<=rx2<w:
                    ...
                    L_new = np.hypot(p2[0]-p1[0], p2[1]-p1[1])
                    strain = (L_new - self.gauge_L0_mm) / self.gauge_L0_mm
                    metric_value = ""
                    if metric=="Axial Strain":
                        metric_value = f"εyy @pt1: {eyy[ry1,rx1]:.5f}, pt2: {eyy[ry2,rx2]:.5f}"
                    elif metric=="Transverse Strain":
                        metric_value = f"εxx @pt1: {exx[ry1,rx1]:.5f}, pt2: {exx[ry2,rx2]:.5f}"
                    elif metric=="Equivalent Strain":
                        metric_value = f"εeqv @pt1: {eqv[ry1,rx1]:.5f}, pt2: {eqv[ry2,rx2]:.5f}"
                    elif metric=="Disp X (mm)":
                        metric_value = f"ux @pt1: {ux[ry1,rx1]:.3f} mm, pt2: {ux[ry2,rx2]:.3f} mm"
                    elif metric=="Disp Y (mm)":
                        metric_value = f"uy @pt1: {uy[ry1,rx1]:.3f} mm, pt2: {uy[ry2,rx2]:.3f} mm"
                    gauge_text = f"L₀ = {self.gauge_L0_mm:.3f} mm, L = {L_new:.3f} mm, ε = {strain:.6f}\n" + metric_value
            ...
            if self.chk_auto.isChecked():
                if self.chk_freeze.isChecked():
                    if not self.frozen:
                        ...
                        if abs(self.vmax - self.vmin) < 1e-9:
            ...
            # color map overlay with inverted Y-axis (blue = positive values)
            flipped_field = -field  # Invert Y axis logic
            cm = cv2.applyColorMap(norm_uint8(flipped_field, -vmax, -vmin), cv2.COLORMAP_JET)
            ...
            # legend (flipped)
            H = vis.shape[0]
            bar = norm_uint8(np.linspace(-vmax,-vmin,H).astype(np.float32), -vmax, -vmin)
            ...
            # display
            if gauge_text:
                self.gauge_lbl.setText(gauge_text.replace("\n", "\n"))
    ...
    def export_to_excel(self):
        try:
        if self.cur is None or self.roi is None or self.ref_gray is None:
            self.info.setText("No data to export.")
            return

        opts = ["Entire ROI", "Select Sub-Area", "Gauge Points Only"]
        choice, ok = QtWidgets.QInputDialog.getItem(self, "Export Scope", "Choose data to export:", opts, 0, False)
        if not ok: return

        x, y, w, h = self.roi
        sp = self.scale_mm

        # Compute smoothed disp and strain fields
        ux = np.zeros((h,w), np.float32)
        uy = np.zeros((h,w), np.float32)
        wt = np.zeros((h,w), np.float32)
        for (p,), d in zip(self.ref_pts, self.cum_disp):
            iy,ix = int(p[1]), int(p[0])
            if 0<=iy<h and 0<=ix<w:
                ux[iy,ix], uy[iy,ix], wt[iy,ix] = d[0]*sp, d[1]*sp, 1
        k = self.spin_ks.value()|1
        k = min(k, (min(h,w)-1)|1)
        ux = cv2.GaussianBlur(ux,(k,k),0)/(cv2.GaussianBlur(wt,(k,k),0)+1e-6)
        uy = cv2.GaussianBlur(uy,(k,k),0)/(cv2.GaussianBlur(wt,(k,k),0)+1e-6)
        ...
        if choice == "Select Sub-Area":
            sub = cv2.selectROI("Select Sub-Area", self.cur[y:y+h,x:x+w])
            cv2.destroyAllWindows()
            sx, sy, sw, sh = map(int, sub)
            if sw == 0 or sh == 0:
                self.info.setText("Sub-area not valid."); return
            ux, uy = ux[sy:sy+sh, sx:sx+sw], uy[sy:sy+sh, sx:sx+sw]
            exx, eyy, exy, eqv = exx[sy:sy+sh, sx:sx+sw], eyy[sy:sy+sh, sx:sx+sw], exy[sy:sy+sh, sx:sx+sw], eqv[sy:sy+sh, sx:sx+sw]

        elif choice == "Gauge Points Only":
            if not self.gauge_pts or not self.gauge_L0_mm:
                self.info.setText("No gauge points selected.")
                return
            (gx1,gy1),(gx2,gy2) = self.gauge_pts
            rx,ry,_w,_h = self.roi
            ry1,rx1 = gy1-ry, gx1-rx
            ry2,rx2 = gy2-ry, gx2-rx
            if not (0<=ry1<h and 0<=rx1<w and 0<=ry2<h and 0<=rx2<w):
                self.info.setText("Gauge points out of ROI.")
                return
            data = {
                "gx1_x": [gx1], "gx1_y": [gy1],
                "gx2_x": [gx2], "gx2_y": [gy2],
                "disp_x1 (mm)": [ux[ry1,rx1]],
                "disp_y1 (mm)": [uy[ry1,rx1]],
                "disp_x2 (mm)": [ux[ry2,rx2]],
                "disp_y2 (mm)": [uy[ry2,rx2]],
                "strain (gauge)": [(np.hypot((gx2-gx1)+(ux[ry2,rx2]-ux[ry1,rx1]),
                                              (gy2-gy1)+(uy[ry2,rx2]-uy[ry1,rx1])) - self.gauge_L0_mm) / self.gauge_L0_mm]
            }
            ...
            df = pd.DataFrame({
                ...
                "eyy": eyy.flatten(),
                "exy": exy.flatten(),
                "equivalent strain": eqv.flatten()
            })

        fname, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save Excel", "", "Excel Files (*.xlsx)")
        if fname:
            df.to_excel(fname, index=False)
            self.info.setText(f"Exported to {fname}")
        else:
            self.info.setText("Export cancelled.")
        except Exception as e:
