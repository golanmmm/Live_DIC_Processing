...
class DICLive(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # Timer
        QtCore.QTimer.singleShot(100, self.prompt_video_source)
    ...
    def prompt_video_source(self):
        ...
        for i in range(6):
            ...
            if cap.isOpened():
                ...
                cap.release()
        options.append("Enter RTSP/IP Stream...")

        item, ok = QtWidgets.QInputDialog.getItem(self, "Select Video Source",
                                                  "Choose a webcam or IP stream:", options, 0, False)
        if not ok:
            sys.exit()
        ...
        if "Camera" in item:
            ...
            text, ok2 = QtWidgets.QInputDialog.getText(self, "Enter Stream URL", "RTSP or IP Stream:")
            if not ok2:
                sys.exit()
            source = text
        ...
        if not self.cap.isOpened():
            QtWidgets.QMessageBox.critical(self, "Error", f"Failed to open video source: {source}")
            sys.exit()
